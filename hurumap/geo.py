...
from hurumap.data.utils import LocationNotFound
...
class GeoData(object):
    """ General hurumap geography helper object.

    This object helps HURUmap load geographies, navigate geo level hierarchies,
    ...
    from this one), then set the `HURUMAP['geodata']` to the dotted path of your
    new class in your `settings.py`. HURUmap will then load that class and make
    it available as `hurumap.geo.geo_data`.
    """
    def __init__(self):
    ...
    def setup_levels(self):
        """ Setup the summary level hierarchy from the `HURUMAP['levels']` and
        `HURUMAP['comparative_levels']` settings.
        """
        self.comparative_levels = ['this'] + settings.HURUMAP['comparative_levels']
...
geo_data = import_string(settings.HURUMAP['geodata'])()


def gdal_missing(critical=False):
    log.warn("NOTE: HURUmap is unable to load GDAL, it's probably not installed. "
